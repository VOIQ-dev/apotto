    ...
    try {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            try {
                ...
                console.log(`[auto-submit/batch] Completed ${i + 1}/${items.length}: success=${result.success}`);
            }
            catch (error) {
                const message = error instanceof Error ? error.message : String(error);
                res.write(`data: ${JSON.stringify({
...
async function findAndFillFormAnyContext(page, payload, log) {
    if (await findAndFillForm(page, payload, log))
        return true;
    for (const frame of page.frames()) {
        if (frame === page.mainFrame())
            continue;
        if (await findAndFillForm(frame, payload, log))
            return true;
    }
    return false;
}
