...
- `src/lib/openaiClient.ts` のプロンプトテンプレートを Sender / Recipient / Product Context / Target Site Summary / Attached PDFs で区切る。AIモデルはGPT-5-mini（フォールバック: GPT-5-nano）。
...
6. 各企業のサイトを開く → ページ種別判定 → フォーム検出（スコアリング方式） → 入力 → 送信
7. すべての処理が完了すると、リード表に結果を自動反映

#### フォーム自動入力の対応項目

- **基本フィールド**: 会社名、担当者名（フルネーム・姓名分割）、メール、電話番号（分割入力対応）、メッセージ
- **日本語特有フィールド**: ふりがな（ひらがな/カタカナ自動判定・変換）、姓名カナ分割
- **住所フィールド**: 郵便番号（オートコンプリート待機）、都道府県（select対応）、市区町村、番地
- **確認用フィールド**: メールアドレス確認用フィールドの自動検出・入力
- **ラベル認識**: 「お名前」「お電話番号」「お問い合わせ内容」等の「お」接頭辞付きラベル対応
- **チェックボックス**: プライバシーポリシー同意、選択式グループ（優先/回避キーワードで自動選択）
- **ラジオボタン**: 「その他」「一般」等を優先、「採用」「資料請求」等を回避
- **セレクトボックス**: 未選択の必須セレクトボックスの自動選択
- **部署・役職**: 部署名、役職名フィールドの自動入力
- **件名**: お問い合わせ件名フィールドの自動入力

#### ページ除外機能

以下のページタイプは自動的に送信対象外：

- **ハードブロック**: 採用・求人、IR/投資家、資料ダウンロード、ニュースレター登録
- **ソフトブロック**: 見積り・トライアル・デモ申込

#### CAPTCHA検出

- reCAPTCHA v2（チェックボックス型）: 送信不可（isBlocker: true）
- reCAPTCHA v2（Invisible）: 送信可能
- reCAPTCHA v3: 送信可能（スパム判定リスクあり）
- hCaptcha: 送信不可
- Cloudflare Turnstile: 送信不可

#### 確認画面対応

- 確認ボタン検出→クリック→MutationObserverで最終送信ボタン検出→クリック
- 除外テキスト: 「戻る」「修正」「キャンセル」等

#### 送信結果ログ

送信結果は \`server/submission-results.log\` に自動記録。
失敗カテゴリ: FORM_NOT_FOUND, BUTTON_NOT_FOUND, VALIDATION_ERROR, CAPTCHA_BLOCKED, CAPTCHA_SPAM, ALERT_BLOCKED, DISABLED_BUTTON, SUCCESS_UNCONFIRMED, TIMEOUT, CONTENT_SCRIPT_ERROR, UNKNOWN
...
- 同時送信数（1-10）を変更でき、設定値が拡張機能に正しく反映されること。
- 並行数に応じて、複数のブラウザタブが同時に開いてフォーム送信を実行すること。
- 送信中は並行数の変更とログ削除が無効化されること。

### フォーム送信

- 企業のホームページURLから自動的に問い合わせフォームをスコアリング方式で検出できること。
- フォームに正しく入力され、送信ボタンをクリックできること。
- 送信完了ページ（thanksページ等）を正しく検出できること。
- メールアドレス確認用フィールド、ふりがなフィールド（ひらがな/カタカナ自動判定）、必須チェックボックスが正しく処理されること。
- 「お名前」等の「お」接頭辞付きラベルが正しく認識されること。
- 姓名分割、電話番号分割、住所分割（郵便番号・都道府県・市区町村・番地）が正しく処理されること。
- ページ種別判定（ハードブロック/ソフトブロック）が正しく動作すること。
- CAPTCHA検出が正しく動作し、送信不可のCAPTCHAでは送信をスキップすること。
