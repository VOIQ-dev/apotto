...
営業文面生成APIは、相手企業のWebサイトURLと送信者・受信者情報を指定するだけで、AI（GPT-5-mini）が自動的に問い合わせフォーム向けの営業文面を生成します。

### 主な機能

- Jina AI Reader + フォールバックfetchによる企業Webサイトの情報抽出（タイムアウト30秒）
- OpenAI GPT-5-miniによる自然な営業文面生成（フォールバック: GPT-5-nano）
- カスタマイズ可能なトーン（フレンドリー/フォーマル/カジュアル）
- 添付資料の自動挿入
- 商談日程URL（Calendlyなど）の組み込み
- OpenAI失敗時の自動フォールバック（ローカルテンプレート生成）
- 最大5件の同時AI生成
...
- 指定したURLにアクセスできない
- タイムアウト（Jina Reader: 30秒、基本フェッチ: 10秒）
...
営業文面生成APIを使えば、相手企業のURLを指定するだけで、AI（GPT-5-mini）が自動的に企業情報を分析し、自然で説得力のある問い合わせフォーム向けの営業文面を生成できます。
