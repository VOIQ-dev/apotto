...
- ✅ ふりがながひらがな/カタカナで正しく入力される（ラベル・placeholder・name属性から自動判定）
- ✅ 姓名が分割フィールドに正しく入力される（姓/名、せい/めい）
- ✅ 郵便番号、都道府県（select対応）、市区町村、番地が正しく個別入力される
- ✅ 電話番号がハイフン付きで入力される（分割フィールド対応: 2〜4分割）
- ✅ メールアドレス確認用フィールドにも入力される
- ✅ 「お名前」「お電話番号」等の「お」接頭辞付きラベルが正しく認識される
- ✅ プライバシーポリシー同意チェックボックスが自動チェックされる
- ✅ ラジオボタンが適切に選択される（「その他」「一般」を優先、「採用」を回避）

### 3.2.1 ページ除外テスト

**テスト対象**: 採用ページ、資料ダウンロードページなど

**テスト手順**:

1. 採用ページURL（例: example.com/recruit）を持つリードを送信
2. 結果を確認

**期待結果**:

- ✅ ハードブロック: 採用・求人、IR/投資家、資料ダウンロード、ニュースレター登録ページが除外される
- ✅ ソフトブロック: 見積り・トライアル・デモ申込ページが除外される
- ✅ お問い合わせページ（/contact, /inquiry等）は除外されない

### 3.2.2 CAPTCHA検出テスト

**テスト対象**: CAPTCHAが設置されたフォーム

**テスト手順**:

1. reCAPTCHA v2（チェックボックス型）のあるサイトを送信
2. reCAPTCHA v3のあるサイトを送信
3. 結果を確認

**期待結果**:

- ✅ reCAPTCHA v2（チェックボックス型）: CAPTCHA_BLOCKED で送信不可
- ✅ reCAPTCHA v2（Invisible）: 送信試行される
- ✅ reCAPTCHA v3: 送信試行される（スパム判定の場合は CAPTCHA_SPAM）
- ✅ hCaptcha / Cloudflare Turnstile: CAPTCHA_BLOCKED で送信不可
...
| 問題                                           | 影響度 | 対処法                 | ステータス |
| ---------------------------------------------- | ------ | ---------------------- | ---------- |
| 一部サイトでフォーム未検出（FORM_NOT_FOUND）   | Medium | スコアリング閾値の調整 | Open       |
| 外部フォームサービス（Googleフォーム等）非対応 | Low    | 対応予定なし           | Won't Fix  |
| reCAPTCHA v2チェックボックス型は送信不可       | Low    | 手動送信               | Won't Fix  |
| hCaptcha / Cloudflare Turnstileは送信不可      | Low    | 手動送信               | Won't Fix  |
| reCAPTCHA v3でスパム判定される場合がある       | Low    | 送信タイミングの調整   | Open       |
| 生年月日等の特殊フィールドは未対応             | Low    | B2B向けフォームでは稀  | Won't Fix  |
