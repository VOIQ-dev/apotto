...
- Jina AI Reader APIによる構造化抽出（タイムアウト: 30秒）
- 同一オリジンのみ: true

**フォールバック機能**: Jina API失敗時は基本的なHTTPフェッチ（タイムアウト: 10秒）で代替取得を試行します。
...
- API全体: 最大120秒
- Webクローリング: 最大30秒（Jina API）、10秒（基本フェッチ）
...
- **OpenAI API**: GPT-5-miniモデルによる文案生成（フォールバック: GPT-5-nano）
- **Jina AI Reader API**: Webページの構造化抽出（タイムアウト30秒）
- **ランタイム**: Node.js
- **最大実行時間**: 120秒
