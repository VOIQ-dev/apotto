...
export default function BillingPage() {
  ...
  const stripeEnabled = useMemo(
    () =>
      (process.env.NEXT_PUBLIC_ENABLE_STRIPE || "").toLowerCase() === "true",
    [],
  );
      ...
      <div className="mx-auto max-w-3xl rounded-2xl bg-white p-6 shadow-lg ring-1 ring-slate-100">
        <header className="mb-6 border-b border-slate-200 pb-4">
          ...
          <p className="mt-1 text-sm text-slate-600">
            Stripe Checkout
            へ遷移して購入します。スタブ設定の場合はモックURLにリダイレクトします。
          </p>
          {!stripeEnabled && (
            <p className="mt-2 text-sm text-amber-600">
              現在 ENABLE_STRIPE=false
              のため、モックURLに遷移します（課金は発生しません）。
            </p>
          )}
        </header>
          ...
          {PLANS.map((p) => (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                {plan === p.id && (
                  <span className="text-xs rounded-full bg-slate-900 px-2 py-1 text-white">
                    選択中
                  </span>
                )}
              </div>
        ...
        <section className="mt-6 space-y-4">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              Stripe metadata と subscriptions に保存されます。
            </p>
          </div>
          <div>
            <label className="text-sm font-medium text-slate-700">
