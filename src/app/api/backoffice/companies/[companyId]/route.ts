...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    if (body.domain !== undefined)
      payload.domain = String(body.domain).trim() || null;
    if (body.status !== undefined)
      payload.status = String(body.status).trim() || "active";
...
export async function DELETE(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    if (accountsErr) {
      console.error(
        "[backoffice/companies] accounts lookup failed",
        accountsErr,
      );
    } else {
      for (const row of accounts ?? []) {
        const email = String((row as { email?: unknown }).email ?? "")
          .trim()
          .toLowerCase();
        if (!email) continue;
        try {
          ...
          console.warn(
            "[backoffice/companies] delete auth user failed",
            email,
            e,
          );
        }
