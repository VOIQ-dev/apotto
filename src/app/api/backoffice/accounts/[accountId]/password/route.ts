...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    if (!id) {
      return NextResponse.json({ error: 'accountId が不正です' }, { status: 400 });
    }
    ...
    if (accountErr) {
      console.error('[backoffice/accounts/password] accounts lookup failed', accountErr);
    ...
    const email = String((account as { email?: unknown }).email ?? '')
      ...
      .toLowerCase();
    if (!email) {
      return NextResponse.json({ error: 'アカウント情報が不正です' }, { status: 400 });
    }
    ...
    if (userErr) {
      console.error('[backoffice/accounts/password] getUserById failed', userErr);
      return NextResponse.json({ error: '再発行に失敗しました' }, { status: 500 });
    }

    const prevMeta = (userData.user?.user_metadata ?? {}) as Record<string, unknown>;
    const companyId = String((account as { company_id?: unknown }).company_id ?? '');
    ...
    const { error: updateErr } = await (supabase.auth.admin.updateUserById as any)(userId, {
      password,
      email_confirm: true,
      user_metadata: {
        ...prevMeta,
    ...
    if (updateErr) {
      console.error('[backoffice/accounts/password] updateUserById failed', updateErr);
      return NextResponse.json({ error: '再発行に失敗しました' }, { status: 500 });
    }
