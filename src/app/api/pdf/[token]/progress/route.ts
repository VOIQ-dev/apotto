...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    ) as { is_deleted?: unknown } | null | undefined;
    ...
    const { data: existing, error: existingError } = await supabase
      ...
      .select(
        "read_percentage_max, max_page_reached, elapsed_seconds_max, elapsed_seconds_total",
      )
    ...
    const prevMax = existing?.elapsed_seconds_max ?? 0;
    const prevTotal = existing?.elapsed_seconds_total ?? 0;

    const nextRead = Math.max(
      existing?.read_percentage_max ?? 0,
      readPercentage,
    );
    const nextPage = Math.max(existing?.max_page_reached ?? 1, maxPageReached);
    const nextElapsed = Math.max(prevMax, elapsedSeconds);

    // 差分計算（今回の経過時間 - 前回の最大値）で累積加算
    // 例: 前回max=10秒、今回=15秒 → 差分5秒を加算
    const delta = Math.max(0, elapsedSeconds - prevMax);
      ...
      .upsert(
        {
          ...
          elapsed_seconds_total: nextTotal,
        },
