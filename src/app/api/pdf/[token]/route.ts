...
export async function GET(request: NextRequest, context: RouteContext) {
  try {
    ...
    const { data: sendLog, error: dbError } = await supabase
      .from("pdf_send_logs")
      .select(
        "*, pdf:pdfs(storage_path, original_filename, size_bytes, is_deleted)",
      )
      .eq("token", token)
      .single();
...
export async function DELETE(request: NextRequest, context: RouteContext) {
  try {
    ...
    const cookieMutations = ctx.cookieMutations;
    ...
    const scopedCompanyId = ctx.companyId;
