...
export async function POST(request: NextRequest) {
  try {
    ...
    const ownedSet = new Set((ownedPdfs ?? []).map((r) => String((r as { id: unknown }).id)));
    const missing = pdfIds.filter((id) => !ownedSet.has(id));
    if (missing.length > 0) {
      return json(
        { error: '会社に紐づかないPDFが含まれています', missingPdfIds: missing },
        { status: 400 }
      );
    }
    ...
    const { error: insertError } = await supabase.from('pdf_send_logs').insert(rows);
    if (insertError) {
    ...
    const grouped = new Map<string, { day: string; pdfId: string; count: number }>();
    ...
    for (const g of grouped.values()) {
      ...
      if (!inc.ok) {
        console.warn('[pdf/send-log] increment_pdf_daily_metrics failed', inc.error);
      }
    }

    return json({ success: true, count: normalized.length });
  } catch (err) {
