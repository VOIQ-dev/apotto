...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    if (updates.sendStatus !== undefined) dbUpdates.send_status = updates.sendStatus;
    if (updates.pdfSendLogId !== undefined) dbUpdates.pdf_send_log_id = updates.pdfSendLogId;
    if (updates.isAppointed !== undefined) dbUpdates.is_appointed = updates.isAppointed;
    ...
    if (error) {
      ...
      const res = NextResponse.json({ error: '一括更新に失敗しました' }, { status: 500 });
      applyAuthCookies(res, cookieMutations);
      return res;
    }

    const res = NextResponse.json({ message: '更新成功', updatedCount: ids.length });
    applyAuthCookies(res, cookieMutations);
    return res;
  } catch (err) {
    console.error('[leads/bulk] error', err);
    const res = NextResponse.json({ error: '一括更新処理に失敗しました' }, { status: 500 });
    applyAuthCookies(res, cookieMutations);
    return res;
  }
}
