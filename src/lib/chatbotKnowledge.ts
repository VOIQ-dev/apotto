...
**ステップ2: 送信者情報の設定**
「送信者情報」セクションで、会社名、担当者名（姓・名）、ふりがな、メールアドレス、電話番号、郵便番号・都道府県・市区町村・住所などの必須項目を入力してください。
これらの情報がフォーム自動入力に使用されます。
...
**ステップ6: 送信**
同時送信数（1〜10並行）を設定し、「チェック済み企業へ一括送信」ボタンで拡張機能のキューに追加されます。
設定した並行数に応じて自動的に処理され、**全て完了した後**にリード表に結果が反映されます。
...
2. 「同時送信数」ドロップダウンで並行タブ数を設定（1〜10、推奨は3）
3. 「チェック済み企業へ一括送信」ボタンをクリック
4. 拡張機能のキューに追加されます
5. 設定した並行数に応じて自動的に処理されます

**フォーム自動入力の対応項目**
・会社名、担当者名（姓名分割対応）、メール、電話番号（分割入力対応）
・ふりがな（ひらがな/カタカナ自動判定）、住所（郵便番号・都道府県・市区町村・番地）
・メール確認用フィールド、プライバシーポリシー同意チェックボックス
・「お名前」「お電話番号」等の「お」接頭辞付きラベルにも対応
・確認画面→最終送信の2段階フォームにも対応
